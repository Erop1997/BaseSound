...
def notification(context):
    ...
    length_notify = len(added_songs) + len(added_albums) + len(deleted_tracks) + len(deleted_albums)

    return {'notify_objects': notify_objects, 'path': path, 'deleted_objects':deleted_objects, 'added_songs':added_songs, 'added_albums':added_albums, 'deleted_tracks':deleted_tracks, 'deleted_albums':deleted_albums, 'length_notify':length_notify}